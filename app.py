...
def api_gantt_site(site_id):
    ...
    if not site:
        conn.close()
        return jsonify({"site": None, "items": []})
    cur.execute(
        '''
...
def calendar_ics():
    ...
    data = resp.get_json()
    lines = ["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//WellAtlasV4//Calendar//EN"]
    import uuid
    for ev in data:
        start = (ev["start"] or "").replace("-", "")
        end = (ev["end"] or ev["start"] or "").replace("-", "")

...
# Create new site
@app.post('/api/add_site')
def api_add_site():
    data = request.get_json(force=True)
    cid = data.get('customer_id'); name = data.get('name'); desc = data.get('description','')
    lat = data.get('latitude'); lon = data.get('longitude')
    if not (cid and name and lat is not None and lon is not None):
        abort(400)
    c=db();cur=c.cursor()
    cur.execute('INSERT INTO sites(customer_id,name,description,latitude,longitude) VALUES(?,?,?,?,?)',(cid,name,desc,lat,lon))
    c.commit(); c.close()
    return jsonify({'ok':True})

# List customers & sites for map
...
def api_sites():
    ...
    c=db();cur=c.cursor()
    sql = 'SELECT sites.*, customers.name as customer FROM sites JOIN customers ON customers.id=sites.customer_id LEFT JOIN jobs ON jobs.site_id=sites.id'
    wh=[]; params=[]
    if customer_id: wh.append('customers.id=?'); params.append(customer_id)
    if category: wh.append('jobs.job_category=?'); params.append(category)
    if q:
        like=f'%{q}%'
    ...
    sql += ' GROUP BY sites.id ORDER BY customers.name, sites.name'
...
def api_gantt_global():
    ...
    c.close()
    colors={"Drilling":"#b8860b","Electrical":"#daa520","Domestic":"#cd853f","Ag":"#deb887", None:"#8a8a8a"}
